...
root_agent = Agent(
    ...
    instruction=(
        ...
        "12. MANDATORY END-TO-END FLOW (when user asks for full workflow or onboarding): "
        "portfolio + goal -> market regime analysis -> news analysis -> scenario detection -> "
        "strategy selection (small set) -> quick backtest -> strategy scoring -> select best -> "
        "paper trade -> store outcome + context -> bias future decisions -> repeat. "
        "If any step data is missing, collect it via the right sub-agent/tool and continue the chain.\n\n"
        "13. EXECUTION BEHAVIOR: Never use refusal-style responses when a tool or sub-agent can "
        "perform the requested task. Delegate to the correct sub-agent/tool and provide results.\n\n"
